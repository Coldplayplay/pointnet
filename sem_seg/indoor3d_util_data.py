...
DATA_PATH = os.path.join(ROOT_DIR, 'data', '5081_data')
...
#####################################################################
def collect_point(anno_path, out_filename, file_format='txt'):
    """ Convert original dataset files to data_label file (each line is XYZRGBL).
        We aggregated all the points from each instance in the room.
...
def sample_data_label(data, num_sample):
    new_data, sample_indices = sample_data(data, num_sample)
    #new_label = label[sample_indices]
    return new_data


def room2blocks(data, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)):
        ...
        block_data_sampled = \
            sample_data_label(block_data, num_point)
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
    ...
    data_batch = room2blocks(data, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug)
